...
export class WeatherForecastRequest {
    ...
    public static defaultLifeTime = 180 * 1000 * 60; // 60 minutes
    ...
    private static requestForecast(location: Location, key: string): Promise<any> {
        console.log(`Fetching data for ${location.longitude}, ${location.latitude}`);
        return fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${location.latitude}&lon=${location.longitude}&appid=${key}`);
    }
    ...
    public async getForecastData() {
        ...
        let response: Response;
        do {
            key = WeatherForecastRequest.getWorkingKey();
            if (key == null) {
                return {
                    error : 529,
                    message : 'No working keys.',
            ...
            response = await WeatherForecastRequest.requestForecast(this.location, key.value);
    ...
    private static getWorkingKey(): Key | null {
        for (let i = 0; i < WeatherForecastRequest.keys.length; i++) {
            if (WeatherForecastRequest.keys[i].isValid) {
                return WeatherForecastRequest.keys[i];
            }
        }
        return null;
    }
